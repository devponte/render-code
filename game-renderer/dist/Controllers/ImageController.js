...
const box = new QueueBox(`ImageBox`, Config.Ports.RCC.Image);
router.post("/image", Valid(AssetRenderRequest), async (req, res) => {
    ...
    return await box.Enqueue((port) => RequestRCCBase(req, res, xml, port, req.body.isFace ? "Face" : "Image"));
});
router.post("/clothing", Valid(AssetRenderRequest), async (req, res) => {
    ...
    return await box.Enqueue((port) => RequestRCCBase(req, res, xml, port, "Clothing"));
});
router.post("/teeshirt", Valid(AssetRenderRequest), async (req, res) => {
    ...
    const base64Image = await box.Enqueue((port) => RequestRCCBaseXMLData(req, res, xml, port, "TeeShirt", 2));
