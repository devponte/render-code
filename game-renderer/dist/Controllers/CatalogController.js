...
const box = new QueueBox(`CatalogBox`, Config.Ports.RCC.Catalog);
router.post("/hat", Valid(AssetRenderRequest), async (req, res) => {
    ...
    return await box.Enqueue((port) => RequestRCCBase(req, res, xml, port, "Catalog hat"));
});
router.post("/head", Valid(AssetRenderRequest), async (req, res) => {
    ...
    return await box.Enqueue((port) => RequestRCCBase(req, res, xml, port, "Catalog head"));
});
router.post("/model", Valid(AssetRenderRequest), async (req, res) => {
    const xml = JSON.parse(JSON.stringify(ModelTemplate));
    xml.Settings.Arguments[0] = `${Config.BaseUrl}/v1/asset?id=${req.body.assetId}`;
    xml.Settings.Arguments[4] = Config.BaseUrl;
    Console.Debug(`Queueing catalog model with AssetId ${req.body.assetId}`);
    return await box.Enqueue((port) => RequestRCCBase(req, res, xml, port, "Catalog model"));
});
router.post("/mesh", Valid(AssetRenderRequest), async (req, res) => {
    ...
    return await box.Enqueue((port) => RequestRCCBase(req, res, xml, port, "Catalog mesh"));
});
router.post("/package", Valid(PackageRenderRequest), async (req, res) => {
    ...
    return await box.Enqueue((port) => RequestRCCBase(req, res, xml, port, "Catalog package"));
});
router.post("/bodypart", Valid(BodyPartRenderRequest), async (req, res) => {
    ...
    return await box.Enqueue((port) => RequestRCCBase(req, res, xml, port, "Body part request"));
});
router.post("/animationsilhouette", Valid(AssetRenderRequest), async (req, res) => {
    const xml = JSON.parse(JSON.stringify(AnimSilhouetteTemplate));
    xml.Settings.Arguments[0] = `${Config.BaseUrl}/v1/asset?id=${req.body.assetId}`;
    xml.Settings.Arguments[1] = Config.BaseUrl;
    xml.Settings.Arguments[4] = '128/128/128';
    Console.Debug(`Queueing catalog animation silhouette with AssetId ${req.body.assetId}`);
    return await box.Enqueue((port) => RequestRCCBase(req, res, xml, port, "Catalog animation silhouette"));
});
router.post("/animation", Valid(AnimationRenderRequest), async (req, res) => {
    ...
    return await box.Enqueue((port) => RequestRCCBase(req, res, xml, port, "Catalog animation"));
});
